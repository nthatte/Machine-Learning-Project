function [sol,b,obj] = primal_svm(linear,Y,lambda,X,wstart,opt)
...
% Minor Modifications by Tomasz Malisiewicz (tomasz@cmu.edu)
  ...
  if  linear 
    ...
    if exist('wstart','var')
      wstartold = wstart;
      wstart(1:(end-1)) = wstartold(2:end);
      wstart(end) = wstartold(1);
    ...
    if ~opt.cg
      [sol,obj] = primal_svm_linear   (Y,lambda,X,opt,wstart);
    else
      [sol,obj] = primal_svm_linear_cg(Y,lambda,X,opt);
    end;
...
function  [w,obj] = primal_svm_linear(Y,lambda,X,opt,wstart) 
% -------------------------------
...
% -------------------------------
  ...
  while 1
    ...
    [obj, grad, sv] = obj_fun_linear(w,Y,lambda,X,out);      
...
function  [w, obj] = primal_svm_linear_cg(Y,lambda,X,opt)
% -----------------------------------------------------
...
% -----------------------------------------------------
  ...
  while 1
    ...
    % Do an exact line search
    [t,out] = line_search_linear(w,s,out,Y,X,lambda);
    w = w + t*s;
      
    % Compute the new gradient
    [obj, gn] = obj_fun_linear(w,Y,lambda,X,out); gn=-gn;
...
function [obj, grad, sv] = obj_fun_linear(w,Y,lambda,X,out)
